...
static int set_int_enable(unsigned char enable)
{
    ...
    if (st.chip_cfg.dmp_on) {
        if (enable)
            ...
            tmp = 0x00;
        if (SPI_Mem_Write(CS_IMU_Pin, st.reg->int_enable, 1, &tmp))
            return -1;
...
int mpu_reg_dump(void)
{
    ...
    for (ii = 0; ii < st.hw->num_reg; ii++) {
        ...
        //    return -1;
        //log_i("%#5x: %#5x\r\n", ii, data);
        SPI_Mem_Read(CS_IMU_Pin, ii, 1, [](uint8_t *data){ log_i("%#5x: %#5x\r\n", ii, data); });
    }
    return 0;
}
...
int mpu_read_reg(unsigned char reg, unsigned char *data)
{
    ...
    SPI_Mem_Read(CS_IMU_Pin, reg, 1, [](uint8_t *data){ log_i("%#5x: %#5x\r\n", ii, data); });
}
...
int mpu_init(struct int_param_s *int_param)
{
    ...
    /* Reset device. */
    data[0] = BIT_RESET;
    if (SPI_Mem_Write(CS_IMU_Pin, st.reg->pwr_mgmt_1, 1, data))
        return -1;
    delay_ms(100);
   ...
   st.chip_cfg.accel_half = 0;
    ...
    if (SPI_Mem_Write(CS_IMU_Pin, st.reg->accel_cfg2, 1, data))
        return -1;
#endif
...
int mpu_lp_accel_mode(unsigned short rate)
{
    ...
    if (!rate) {
        ...
        tmp[0] = 0;
        tmp[1] = BIT_STBY_XYZG;
        if (SPI_Mem_Write(CS_IMU_Pin, st.reg->pwr_mgmt_1, 2, tmp))
            return -1;
        st.chip_cfg.lp_accel_mode = 0;
        return 0;
    }
...
int mpu_get_gyro_reg(short *data, unsigned long *timestamp)
{
    ...
    SPI_Mem_Read(CS_IMU_Pin, st.reg->raw_gyro, 6, [](uint8_t *tmp){
      data[0] = (tmp[0] << 8) | tmp[1];
      data[1] = (tmp[2] << 8) | tmp[3];
      data[2] = (tmp[4] << 8) | tmp[5];
    });
...
int mpu_get_accel_reg(short *data, unsigned long *timestamp)
{
    ...
    //data[1] = (tmp[2] << 8) | tmp[3];
    //data[2] = (tmp[4] << 8) | tmp[5];
    SPI_Mem_Read(CS_IMU_Pin, st.reg->raw_accel, 6, [](uint8_t *tmp){
      data[0] = (tmp[0] << 8) | tmp[1];
      data[1] = (tmp[2] << 8) | tmp[3];
      data[2] = (tmp[4] << 8) | tmp[5];
    });
...
int mpu_get_temperature(long *data, unsigned long *timestamp)
{
    ...
    //    return -1;
    //raw = (tmp[0] << 8) | tmp[1];
    SPI_Mem_Read(CS_IMU_Pin, st.reg->temp, 2, [](uint8_t *tmp){ raw = (tmp[0] << 8) | tmp[1]; });
    if (timestamp)
        get_ms(timestamp);
...
int mpu_set_gyro_bias_reg(long *gyro_bias)
{
    ...
    if (SPI_Mem_Write(CS_IMU_Pin, 0x13, 2, &data[0]))
        return -1;
    if (SPI_Mem_Write(CS_IMU_Pin, 0x15, 2, &data[2]))
        return -1;
    if (SPI_Mem_Write(CS_IMU_Pin, 0x17, 2, &data[4]))
        return -1;
    return 0;
}
...
int mpu_reset_fifo(void)
{
    ...
    data = 0;
    if (SPI_Mem_Write(CS_IMU_Pin, st.reg->int_enable, 1, &data))
        return -1;
    if (SPI_Mem_Write(CS_IMU_Pin, st.reg->fifo_en, 1, &data))
        return -1;
    if (SPI_Mem_Write(CS_IMU_Pin, st.reg->user_ctrl, 1, &data))
        return -1;
    ...
    if (st.chip_cfg.dmp_on) {
        ...
        if (SPI_Mem_Write(CS_IMU_Pin, st.reg->int_enable, 1, &data))
            return -1;
        if (SPI_Mem_Write(CS_IMU_Pin, st.reg->fifo_en, 1, &st.chip_cfg.fifo_enable))
            return -1;
    }
    return 0;
}
...
int mpu_set_gyro_fsr(unsigned short fsr)
{
    ...
    if (st.chip_cfg.gyro_fsr == (data >> 3))
        return 0;
    if (SPI_Mem_Write(CS_IMU_Pin, st.reg->gyro_cfg, 1, &data))
        return -1;
...
int mpu_set_accel_fsr(unsigned char fsr)
{
    ...
    if (st.chip_cfg.accel_fsr == (data >> 3))
        return 0;
    if (SPI_Mem_Write(CS_IMU_Pin, st.reg->accel_cfg, 1, &data))
        return -1;
...
int mpu_set_lpf(unsigned short lpf)
{
    ...
    if (st.chip_cfg.lpf == data)
        return 0;
    if (SPI_Mem_Write(CS_IMU_Pin, st.reg->lpf, 1, &data))
        return -1;
...
int mpu_set_sample_rate(unsigned short rate)
{
    ...
    if (st.chip_cfg.dmp_on)
        ...
        data = 1000 / rate - 1;
        if (SPI_Mem_Write(CS_IMU_Pin, st.reg->rate_div, 1, &data))
            return -1;
...
int mpu_set_compass_sample_rate(unsigned short rate)
{
    ...
    if (SPI_Mem_Write(CS_IMU_Pin, st.reg->s4_ctrl, 1, &div))
        return -1;
...
int mpu_set_sensors(unsigned char sensors)
{
    ...
    if (!(sensors & INV_XYZ_ACCEL))
        data |= BIT_STBY_XYZA;
    if (SPI_Mem_Write(CS_IMU_Pin, st.reg->pwr_mgmt_2, 1, &data)) {
        st.chip_cfg.sensors = 0;
        return -1;
    }
    ...
    //else
    //    user_ctrl &= ~BIT_DMP_EN;
    //if (SPI_Mem_Write(CS_IMU_Pin, st.reg->s1_do, 1, &data))
    //    return -1;
    /* Enable/disable I2C master mode. */
    //if (SPI_Mem_Write(CS_IMU_Pin, st.reg->user_ctrl, 1, &user_ctrl))
    //    return -1;
    SPI_Mem_Read(CS_IMU_Pin, st.reg->user_ctrl, 1, [](uint8_t *user_ctrl){
      /* Handle AKM power management. */
      if (sensors & INV_XYZ_COMPASS) {
          data = AKM_SINGLE_MEASUREMENT;
          user_ctrl |= BIT_AUX_IF_EN;
...
int mpu_get_int_status(short *status)
{
    ...
    //    return -1;
    SPI_Mem_Read(CS_IMU_Pin, st.reg->dmp_int_status, 2, [](uint8_t *tmp){ status[0] = (tmp[0] << 8) | tmp[1];});
    return 0;
}
        ...
        unsigned char *sensors, unsigned char *more)
{
    ...
    SPI_Mem_Read(CS_IMU_Pin, st.reg->fifo_count_h, 2, [](uint8_t *data){
      fifo_count = (data[0] << 8) | data[1];
      if (fifo_count < packet_size)
        return 0;
      if (fifo_count > (st.hw->max_fifo >> 1)) {
        /* FIFO is 50% full, better check overflow bit. */
        SPI_Mem_Read(CS_IMU_Pin, st.reg->int_status, 1, [](uint8_t *data){
      ...
      SPI_Mem_Read(CS_IMU_Pin, st.reg->fifo_r_w, packet_size, [](uint8_t *data){
    ...
    unsigned char *more)
{
    ...
    SPI_Mem_Read(CS_IMU_Pin, st.reg->fifo_count_h, 2, [](uint8_t *tmp){
      ...
      if (fifo_count > (st.hw->max_fifo >> 1)) {
        /* FIFO is 50% full, better check overflow bit. */
        SPI_Mem_Read(CS_IMU_Pin, st.reg->int_status, 1, [](uint8_t *tmp){
      ...
      SPI_Mem_Read(CS_IMU_Pin, st.reg->fifo_r_w, length, [](uint8_t *data){
...
int mpu_set_bypass(unsigned char bypass_on)
{
    ...
    if (bypass_on) {
      SPI_Mem_Read(CS_IMU_Pin, st.reg->user_ctrl, 1, [](uint8_t *tmp){
        tmp &= ~BIT_AUX_IF_EN;
        if (SPI_Mem_Write(CS_IMU_Pin, st.reg->user_ctrl, 1, &tmp))
            return -1;
        delay_ms(3);
        tmp = BIT_BYPASS_EN;
        ...
        if (SPI_Mem_Write(CS_IMU_Pin, st.reg->int_pin_cfg, 1, &tmp))
            return -1;
      });
    } else {
...
static int get_st_biases(long *gyro, long *accel, unsigned char hw_test)
{
    ...
    data[0] = 0x01;
    data[1] = 0;
    if (SPI_Mem_Write(CS_IMU_Pin, st.reg->pwr_mgmt_1, 2, data))
        return -1;
    delay_ms(200);
    data[0] = 0;
    if (SPI_Mem_Write(CS_IMU_Pin, st.reg->int_enable, 1, data))
    ...
    if (SPI_Mem_Write(CS_IMU_Pin, st.reg->pwr_mgmt_1, 1, data))
        return -1;
    if (SPI_Mem_Write(CS_IMU_Pin, st.reg->i2c_mst, 1, data))
        return -1;
    if (SPI_Mem_Write(CS_IMU_Pin, st.reg->user_ctrl, 1, data))
        return -1;
    ...
    SPI_Mem_Read(CS_IMU_Pin, st.reg->fifo_count_h, 2, [](uint8_t *data){
      ...
      for (ii = 0; ii < packet_count; ii++) {
        ...
        SPI_Mem_Read(CS_IMU_Pin, st.reg->fifo_r_w, MAX_PACKET_LENGTH, [](uint8_t *data){
        ...
        unsigned char *data)
{
    ...
    if (SPI_Mem_Write(CS_IMU_Pin, st.reg->bank_sel, 2, tmp))
        return -1;
    if (SPI_Mem_Write(CS_IMU_Pin, st.reg->mem_r_w, length, data))
        return -1;
    return 0;
}
...
#define LOAD_CHUNK  (16)
    ...
    if (SPI_Mem_Write(CS_IMU_Pin, st.reg->prgm_start_h, 2, tmp))
        return -1;
...
int mpu_set_dmp_state(unsigned char enable)
{
    ...
    if (enable) {
        ...
        tmp = 0;
        SPI_Mem_Write(CS_IMU_Pin, 0x23, 1, &tmp);
...
static int setup_compass(void)
{
    ...
    if (SPI_Mem_Write(CS_IMU_Pin, st.reg->s0_addr, 1, data))
        return -1;

    /* Compass reads start at this register. */
    data[0] = AKM_REG_ST1;
    if (SPI_Mem_Write(CS_IMU_Pin, st.reg->s0_reg, 1, data))
        return -1;

    /* Enable slave 0, 8-byte reads. */
    data[0] = BIT_SLAVE_EN | 8;
    if (SPI_Mem_Write(CS_IMU_Pin, st.reg->s0_ctrl, 1, data))
        return -1;

    /* Slave 1 changes AKM measurement mode. */
    data[0] = st.chip_cfg.compass_addr;
    if (SPI_Mem_Write(CS_IMU_Pin, st.reg->s1_addr, 1, data))
        return -1;

    /* AKM measurement mode register. */
    data[0] = AKM_REG_CNTL;
    if (SPI_Mem_Write(CS_IMU_Pin, st.reg->s1_reg, 1, data))
        return -1;

    /* Enable slave 1, 1-byte writes. */
    data[0] = BIT_SLAVE_EN | 1;
    if (SPI_Mem_Write(CS_IMU_Pin, st.reg->s1_ctrl, 1, data))
        return -1;
    ...
    data[0] = 0x03;
    if (SPI_Mem_Write(CS_IMU_Pin, st.reg->i2c_delay_ctrl, 1, data))
        return -1;

#ifdef MPU9150
    /* For the MPU9150, the auxiliary I2C bus needs to be set to VDD. */
    data[0] = BIT_I2C_MST_VDDIO;
    if (SPI_Mem_Write(CS_IMU_Pin, st.reg->yg_offs_tc, 1, data))
        return -1;
#endif
